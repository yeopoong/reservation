...
SUPPORTED_SITES = {"paramus", "bergen"}
SITE_CONFIG = {
    "paramus": {
...
def select_mat_option(filter_control_selector: str, target_text: str, multiple: bool = False) -> bool:
...
def apply_holes_filter(page, target_holes: str = "18 Holes") -> bool:
    ...
    dismiss_search_overlays(page)
    if select_mat_option("app-search-teetime-filters .hole-filter-control", target_holes):
...
def apply_course_filter(page, target_course: str | None) -> bool:
    ...
    dismiss_search_overlays(page)
    if select_mat_option("app-search-teetime-filters .course-filter-control", target_course, multiple=True):
        settle_after_filter_change(page)
        ...
        for attempt in range(1, 301):
            try:
                ...
                try:
                    page.wait_for_function(
                        f"""() => {{
                            const guestCount = Array.from(document.querySelectorAll('mat-checkbox')).filter(
                                cb => cb.querySelector('label')?.textContent?.trim() === 'Guest'
                            ).length;
                            const finalize = Array.from(document.querySelectorAll('button')).some(
                                btn => /Finalize Reservation/i.test(btn.innerText || btn.textContent || '')
                            );
                            return guestCount >= {expected_guests} || finalize;
                        }}""",
                        timeout=6000,
                    )
                except Exception:
                ...
                if click_continue_to_review_if_present(page):
                    ...
                    try:
                        page.wait_for_selector(FINALIZE_SELECTOR, timeout=3000)
                    except Exception:
                        snapshot = get_results_debug_snapshot(page)
                        logger(
